...
#import "ProfilesToolController.h"
#import "ProfilesNode.h"
#import "NSFileManager+Trash.h"
@interface ProfilesToolController ()

@end

@implementation ProfilesToolController
...
- (void)windowDidLoad {
    ...
    self.treeView.menu = self.mainMenu;
//
}
...
- (void)menuWillOpen:(NSMenu *)menu
{
    ...
    if(menu == _itemMenu){
//        NSMenuItem *renameItem = [menu itemWithTag:999];
//        if (!renameItem)
//        {
//            renameItem = [[NSMenuItem alloc] initWithTitle:@"重命名" action:@selector(renameItemClick:) keyEquivalent:@""];
//            [renameItem setTarget:self];
//            [renameItem setTag:999];
...
- (void)renameItemClick:(id)sender
{
    ...
    [[NSFileManager defaultManager] moveItemAtPath:node.filePath toPath:[[_profileDir stringByAppendingString:fileName?:@""] stringByAppendingPathExtension:@"mobileprovision"] error:&error];
    if(error)
    {
        [self showMessage:[error localizedDescription]];
    }
